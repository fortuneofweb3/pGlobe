...
export const GET = withAPIAuth(async (request: Request) => {
  ...
  try {
    console.log('[VercelProxy] Proxying /api/v1/network/health request to backend...');
    
    const response = await fetch(`${RENDER_API_URL}/api/v1/network/health`, {
      method: 'GET',
      headers: {
    ...
    console.log('[VercelProxy] ✅ Returning network health from backend');
    return NextResponse.json(data);
  } catch (error: any) {
    console.error('[VercelProxy] ❌ Failed to proxy /api/v1/network/health to backend:', error);
    return NextResponse.json(
