...
function NodeDetailContent() {
    ...
    const nodeId = params.pubkey as string;
    ...
    const node = useMemo(() => {
        ...
        const foundNode = allMerged.find(m => {
        ...
        // Ensure mergedIPs always exists for the UI switcher, even for single-IP nodes
        if (!foundNode.mergedIPs || foundNode.mergedIPs.length === 0) {
            return {
                ...foundNode,
                isMerged: true,
                mergedIPs: [{
                    ...
                    locationData: foundNode.locationData,
                    credits: foundNode.credits,
                    uptime: foundNode.uptime,
                    storageCapacity: foundNode.storageCapacity,
                    version: foundNode.version
                }]
            };
        }
    ...
    const renderIPSwitcher = () => {
        if (!node) return null;
        // The node now always has mergedIPs thanks to the useMemo update
        if (!node.mergedIPs || node.mergedIPs.length === 0) return null;
