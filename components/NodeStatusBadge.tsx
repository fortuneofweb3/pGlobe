  ...
  const getNodeStatus = () => {
    ...
    if (!isOnline || node.status === 'offline') {
      return {
        ...
        label: 'Critical',
        tooltip: 'pNode is offline or unreachable. This pNode is not responding to network requests.',
    ...
    if (isOnline && !isCurrentVersion) {
      return {
        ...
        label: 'Old Ver',
        tooltip: `pNode is online but running an outdated version (${node.version || 'unknown'}). Latest version is ${latestVersion || 'unknown'}. Consider updating for security and compatibility.`,
    ...
    if (isOnline && isCurrentVersion && hasLowUptime) {
      return {
        ...
        label: 'Warning',
        tooltip: `pNode is online and up-to-date, but has low uptime (${node.uptimePercent?.toFixed(1) || 'unknown'}%). This may indicate instability or frequent restarts.`,
    ...
    if (node.status === 'syncing') {
      return {
        ...
        label: 'Syncing',
        tooltip: 'pNode is currently syncing with the network. This is normal for pNodes that just started or are catching up after downtime.',
    ...
    return {
      ...
      label: 'Healthy',
      tooltip: 'pNode is online, running the latest version, and has good uptime. This pNode is operating normally.',
